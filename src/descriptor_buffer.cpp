...
namespace sve {
	DescriptorBuffer::DescriptorBuffer(VmaAllocator allocator, std::uint32_t const queue_family, vk::BufferUsageFlags const usage)
	...
	void DescriptorBuffer::write_at(std::size_t const frame_index, std::span<std::byte const> bytes) {
		write_to(m_buffers.at(frame_index), bytes);
	}

	vk::DescriptorBufferInfo DescriptorBuffer::descripter_info_at(std::size_t const frame_index) const {
	...
	void DescriptorBuffer::write_to(Buffer& out, std::span<std::byte const> bytes) const {
		...
		if (out.buffer.get().size < bytes.size()) {
			...
			out.buffer = vma::create_buffer(buffer_ci, vma::BufferMemoryType::Host, out.size);
		}
